...
class ScanningTests(unittest.TestCase):
    def test_scan_finds_stack_rules_and_evidence_without_mutation(self):
        with TemporaryDirectory() as directory:
            ...
            self.assertIn(".workbuddy/skills/", report["proposed_writes"])
            ...
            self.assertIn(".workbuddy/skills/ff-*", workbuddy_description)
    ...
    def test_scan_ignores_secrets_dependencies_binary_and_large_files(self):
        with TemporaryDirectory() as directory:
            ...
            (root / ".workbuddy" / "skills").mkdir(parents=True)
            (root / ".workbuddy" / "skills" / "go.mod").write_text("module workbuddy-ignored\n", encoding="utf-8")
            (root / ".workbuddy-ai" / "skills").mkdir(parents=True)
            (root / ".workbuddy-ai" / "skills" / "package.json").write_text('{"name":"legacy-ignored"}', encoding="utf-8")
            ...
            self.assertNotIn(".workbuddy", evidence)
            self.assertNotIn("legacy-ignored", evidence)
